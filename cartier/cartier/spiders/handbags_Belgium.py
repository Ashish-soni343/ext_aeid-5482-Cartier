...
class ExampleSpider(scrapy.Spider):
    name = 'cartier_handbags_BE'

    start_urls = ['https://www.cartier.com/en-be/bags']
    headers = {
    ...
    def parse_details(self, response):
        ...
        if response.css('select option::text').getall():
            ...
            if response.css('p.short-description::text').extract():
                try:
                    colors = response.css('p.short-description::text').extract()
                    container = []
                    str1 = ','
                    for color in colors:
